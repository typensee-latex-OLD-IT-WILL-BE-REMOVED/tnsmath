...
\begin{tcblisting}{}
Est-il vrai que $(a + b)^3 \eqtest a^3 + b^3 + 3 a b$ ? Pour l'algorithmique, 
on dispose d'une écriture standard : $(a + b)^3 \eqtest* a^3 + b^3 + 3 a b$ .

\end{tcblisting}


Le package \verb+lyalgo+ téléchargeable à l'adresse \url{https://github.com/bc-latex/ly-algo} propose quelques outils pour faciliter la saisie d'algorithmes. Ce package charge automatiquement \verb+lymath+.
...
\begin{tcblisting}{}
A-t-on $x \leqtest x^2$ ou $x \lesstest x^2$ ? A moins que ce ne soit $x \geqtest x^2$
ou $x \greattest x^2$.

On peut supposer $x \leqhyp 1$ , $1 \lesshyp x$ ou $1 \greathyp x \geqhyp 0$.
...
% --------------- %
	...
	\begin{itemize}
		\item Ajout de \verb+\eqtest*+ pour des contextes algorithmiques.
		
		\item Différents types de signes d'inéquation pour des cas de test ou d'hypothèse.
	\end{itemize}
